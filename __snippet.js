...
function BuildPane(props){
  const {text,setText,fileRef,onFile,onImport,byProto,search,setSearch,addAfterSelected,
    importInfo,canvas,setCanvas,selected,setSelected,inst,rm,dragIdx,setDragIdx,moveFromTo,addAt,globalEditMode,
    lanesFn, bindings, setBindings, caseName, setCaseName, casesMap, setCasesMap,
    cmpMode, groupMode, currentPage, setCurrentPage, itemsPerPage,
    exportData, exportCaseSelected, importCase, setTab,
    idsText, setIdsText, createCaseByIdsFromText } = props;
  return null;
}
