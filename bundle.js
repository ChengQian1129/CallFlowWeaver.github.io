...
function shortLabel(m) {
  // 锟斤拷锟斤拷锟斤拷 message.name 锟斤拷为锟缴讹拷锟斤拷锟斤拷
...
function aiLog(msg) {
  ...
  try {
    if (typeof window.__aiInlineLog === 'function') window.__aiInlineLog(text);
  } catch (_sink) {}
  try {
...
function aiResetLog(header) {
  ...
  try {
    if (typeof window.__aiInlineReset === 'function') window.__aiInlineReset(text);
  } catch (_sink2) {}
  aiLog(text);
}
function formatWorkerError(res, bodyText) {
  ...
  try {
    ...
    if (parsed && (parsed.error || parsed.message)) {
      var addon = (parsed.error || '') + (parsed.message ? ' 锟?' + parsed.message : '');
  ...
  return base + ' 锟?' + trimmed.slice(0, 500);
}
...
var CASE_GEN_SYSTEM_PROMPT = [
  'IMPORTANT SYSTEM OVERRIDE: Return ONLY one strict JSON object {"message_idx":[...]}.',
  'Indices MUST come from id_to_index; each index maps to a full truth ID in supported_message_ids.',
  'Output Contract: single JSON object, key ONLY "message_idx"; NO code fences, NO explanations, NO extra fields; if none apply, return {"message_idx":[]}.',
  'Selection Source: choose ONLY from supported_message_ids (curated 3GPP/ETSI truth with family/protocol/direction/release).',
  'Forbidden: shorthand names, invented IDs, cross-domain substitutions, any text outside the single JSON object.',
  'Goal: end-to-end coverage of the scenario; include as many spec-recognized messages across involved nodes as applicable; prefer breadth while staying normative.',
  'Flow: keep real chronological order; include request/response legs when present; include optional normative steps when typically required; duplicates allowed if the normative flow repeats; NO length constraints.'
].join('\n');
var CASE_GEN_SYSTEM_PROMPT_FALLBACK = [
  'IMPORTANT SYSTEM OVERRIDE: Return ONLY one strict JSON object {"case_messages":[...]}.',
  'Each item: {"id":"...","from":"...","to":"...","message":"...","ie_tree":[...]}; keep chronological order; NO code fences; NO extra fields.',
  'Do not invent protocol families; use scenario description and common NGAP/NAS names when truth is unavailable.'
].join('\n');
...
var CASE_ANALYZE_SYSTEM_PROMPT = ['You are the CallFlowWeaver IE consistency verifier.', 'Input: {"case":{...},"context":{"scenario":"...","message_ids":[...]}}.', 'For each message, IE specs provide presence (M/C/O), format, enums and examples.', 'Rules:', '1) Use only known IE keys from specs; do not invent keys.', '2) Mandatory (M) must be non-empty and valid.', '3) Conditional (C) depends on scenario and roles; include only when applicable.', '4) Optional (O) may be empty when not needed.', '5) Values must conform to format/enums/ranges; pick reasonable example values within spec.', '6) Keep protocol and directionality intact; do not alter message identity.', 'Output strict JSON {"conflicts":[...],"alignments":{...},"suggestions":[{"uid":"...","message_id":"...","overrides":{"IE_KEY":"VALUE"},"reason":"conflict|align|scenario"},...]}; no explanations; no code fences.'].join('\n');
    ...
    if (Array.isArray(x)) {
      for (var k = 0; k < x.length; k++) {
        ...
        if (y && (y.schema_version || y.id) && (y.ie_tree || y.message || y["interface"])) {
          // 瑙勶拷??message锛氳嫢涓哄璞′笖鍖咃拷??name锛屽垯褰掍竴鍖栦负瀛楃锟?        if(y && typeof y.message==='object' && y.message && y.message.name){ y.message = y.message.name; }
          // 纭拷??id 涓哄瓧绗︿覆??        y.id = String(y.id||'');
          // 鍩虹瀹炰綋缂虹渷
...
function pickRelevantIds(prompt, allIds){
  try{
    ...
    var keys = [];
    if(p.indexOf('娉ㄥ唽')>=0||p.indexOf('registration')>=0){ keys.push('Registration'); keys.push('Setup'); }
...
// 锟斤拷锟斤拷锟斤拷锟斤拷锟接匡拷锟斤拷锟斤拷锟介（锟斤拷锟斤拷使锟斤拷 m.interface锟斤拷锟斤拷锟斤拷贫锟?inferInterface(m.protocol)锟斤拷
...
function App() {
  ...
  var truthSourceRef = React.useRef('imported');
  ...
  var _React$useState65 = React.useState('Idle - awaiting AI tasks'),
    _React$useState66 = _slicedToArray(_React$useState65, 2),
    aiInlineLog = _React$useState66[0],
    setAiInlineLog = _React$useState66[1];
  var _React$useState67 = React.useState(0),
    _React$useState68 = _slicedToArray(_React$useState67, 2),
    aiElapsed = _React$useState68[0],
    setAiElapsed = _React$useState68[1];
  var aiElapsedRef = React.useRef(null);
  ...
  // 锟斤拷 localStorage 锟斤拷取锟斤拷锟斤拷
  React.useEffect(function () {
    try { window.__aiInlineReset = function (line) { setAiInlineLog(String(line || '')); }; } catch (_) {}
    try { window.__aiInlineLog = function (line) {
      setAiInlineLog(function (prev) {
        try {
          var base = String(prev || '');
          var combined = (base ? base + '\n' : '') + String(line || '');
          var parts = combined.split('\n');
          if (parts.length > 160) parts = parts.slice(parts.length - 160);
          return parts.join('\n');
        } catch (_) {
          return String(line || '');
        }
    ...
    return function () {
      try { delete window.__aiInlineLog; delete window.__aiInlineReset; } catch (_) {}
    };
  }, []);
  ...
  React.useEffect(function () {
    var timer = null;
    if (aiBusy) {
      try { aiElapsedRef.current = Date.now(); } catch (_) {}
      setAiElapsed(0);
      timer = setInterval(function () {
        try {
          if (aiElapsedRef.current) {
            setAiElapsed(Math.floor((Date.now() - aiElapsedRef.current) / 1000));
          }
        } catch (_) {}
      }, 1000);
    } else {
  ...
  // 锟斤拷取锟较次碉拷锟斤拷锟斤拷谋锟斤拷锟斤拷锟斤拷荩锟斤拷锟街に拷潞锟斤拷锟?
  React.useEffect(function () {
    try {
    ...
    try {
      ...
      if (rawTruth) {
        ...
        if (Array.isArray(arr)) {
          try { truthSourceRef.current = 'imported'; } catch (_) {}
          setTruth(arr);
  ...
  React.useEffect(function () {
    try {
      ...
      if (truthSourceRef.current === 'imported') {
        if (s.length < 4500000) {
        ...
        try { localStorage.setItem('5gc_derived_truth_meta', JSON.stringify({ count: Array.isArray(truth) ? truth.length : 0, ts: Date.now() })); } catch (_m) {}
      }
    } catch (_) {}
  }, [truth]);
  // 锟斤拷锟捷凤拷锟斤拷模式锟诫导锟斤拷锟斤拷锟捷讹拷态锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
  React.useEffect(function () {
    try {
  ...
  // 锟斤拷取锟斤拷前 Case 锟斤拷锟秸ｏ拷锟斤拷锟狡★拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
  React.useEffect(function () {
    try {
  ...
  // 锟皆讹拷锟斤拷锟芥当前 Case 锟斤拷锟秸碉拷 localStorage
  React.useEffect(function () {
    try {
      var data = {
        caseName: caseName,
  ...
  React.useEffect(function () {
    try {
      if (false) {
        ...
        if (arr.length) {
          try { truthSourceRef.current = 'derived'; } catch (_) {}
          setTruth(arr);
          setByProto(groupMode === 'interface' ? groupByInterface(arr) : groupByProto(arr));
          setImportInfo({ ok: arr.length, err: 0 });
        }
  ...
  // ---- AI锟斤拷锟斤拷 ID 锟斤拷锟斤拷 Case 锟斤拷锟斤拷锟斤拷獠?API ----
  ...
  function buildCaseFromIds(ids) {
    try {
      ...
      var index = {};
      var nameIndex = {};
      var norm = function(s){ try{ return String(s||'').toLowerCase().replace(/[_\-\s]+/g,'').replace(/[^a-z0-9]/g,''); }catch(_){ return ''; } };
      var stripTokens = function(s){ var t=String(s||'').toLowerCase(); ['epc','5gc','nas','ngap','s1ap','diameter','gtpv2c','gtpv1c','map','radius','pfcp','ranap','rrc','ims'].forEach(function(k){ t=t.replace(new RegExp('^'+k+'[_\-\s]*'),''); }); return t; };
      for (var i = 0; i < all.length; i++) {
        var m0 = all[i] || {};
        var mid0 = String(m0 && (m0.id || (m0.info && m0.info.header && m0.info.header.id) || ''));
        if (mid0) index[mid0] = m0;
        try{
          var hdr = (m0 && m0.info && m0.info.header) || {};
          var nm = hdr.msg || m0.message || m0.name || (m0.doc && m0.doc.title) || '';
          var k = norm(nm);
          if(k){ if(!nameIndex[k]) nameIndex[k] = []; nameIndex[k].push(m0); }
        }catch(_){}
      }
      var pick = ids.map(function (id) {
        var k = String(id);
        var exact = index[k] || (window.__truthById && window.__truthById[k]) || null;
        if (exact) return exact;
        var byName = nameIndex[norm(stripTokens(k))] || nameIndex[norm(k)] || [];
  ...
  if (window.showToast) window.showToast('Created case from ID sequence: ' + addItems.length + ' items', 'success');
      ...
      if (window.showToast) window.showToast('Create case by IDs error: ' + (e && e.message || e), 'error');
}
  }
      ...
      return _regeneratorRuntime().wrap(function _callee2$(_context2) {
        ...
        while (1) switch (_context2.prev = _context2.next) {
          ...
          case 0:
            ...
            var urlFallback = String(aiApiUrl || '').trim();
            if(!urlFallback){ try{ urlFallback = String((window && window.__LLM_API_URL) || 'http://127.0.0.1:8787/v1'); }catch(_){ urlFallback='http://127.0.0.1:8787/v1'; } }
            aiApiUrl = urlFallback;
          case 4:
            ...
            // allow generation without imported truth
            ...
            scheduleAutoOptimize = function scheduleAutoOptimize() {
              ...
              try {
                ...
                setTimeout(function () {
                  try { window.__aiSubset = null; } catch (_) {}
                  try { onAiAnalyzeCase(); } catch (_e) {}
                }, 150);
            ...
            if (!scenarioText) {
              try { var taNode = document.querySelector('.cfw-ai-text'); if (taNode && taNode.value) scenarioText = String(taNode.value).trim(); } catch(_){ }
            }
            if (!scenarioText) scenarioText = String(aiBuildText || aiPrompt || '').trim();

            try { if(scenarioText){ window.__aiScenario = scenarioText; if (typeof showAiStatus === 'function') showAiStatus('Scenario: ' + scenarioText); } } catch (_) {}
            ...
            setAiResult('');
            try { window.__aiRaw = ''; } catch (_){ }

            url = String(aiApiUrl || '').trim();
            if(!url){ try{ url = String((window && window.__LLM_API_URL) || 'http://127.0.0.1:8787/v1'); }catch(_){ url='http://127.0.0.1:8787/v1'; } }
            if(!url){ try{ url = String((window && window.__LLM_API_URL) || 'http://127.0.0.1:8787/v1'); }catch(_){ url='http://127.0.0.1:8787/v1'; } }
            ...
            try { aiOpenAiCompat = true; } catch (_) {}
            ...
            if (aiOpenAiCompat) {
              ...
              supportedList = Array.from(new Set(allIds));
              ...
              if (scenarioText) {
                ...
                scenarioUserMsg = scenarioUserPieces.join('\n');
              }
              var rulesMsg = CASE_GEN_RULES;
              if (protoSummary) rulesMsg = rulesMsg + '\n\nProtocol availability: ' + protoSummary;

              var idToIndex = {};
              for (var si = 0; si < supportedList.length; si++) { idToIndex[supportedList[si]] = si; }
              var truthById = {};
              try {
                var allTruth = Array.isArray(truth) ? truth : [];
                for (var ti = 0; ti < allTruth.length; ti++) {
                  var mT = allTruth[ti] || {};
                  var hdrT = mT && mT.info && mT.info.header || {};
                  var idT = String(mT.id || hdrT.id || '');
                  if (idT) truthById[idT] = mT;
                }
              } catch (_mapErr) {}
              try { window.__supportedIds = supportedList; window.__idToIndex = idToIndex; window.__truthById = truthById; } catch (_w) {}

              msgs = [{ role: 'system', content: sys }];
              if (scenarioUserMsg) msgs.push({ role: 'user', content: scenarioUserMsg });
              msgs.push({ role: 'user', content: rulesMsg });
              msgs.push({ role:'user', content: 'id_to_index: ' + JSON.stringify(idToIndex) });
              msgs.push({ role:'user', content: 'Output Contract: Return ONLY a single strict JSON object {"message_idx":[...]} using indices from id_to_index; do NOT output message_ids; if none apply, return {"message_idx":[]}. No code fences; no extra fields.' });
            ...
            try {
              ...
              var reqSummary = ['Generate Case'];
              if (scenarioText) reqSummary.push('Scenario: ' + scenarioText);
              reqSummary.push('supported_ids=' + (supportedList ? supportedList.length : 0));
              reqSummary.push('model=' + (payload && payload.model || aiModel || 'gemini-2.5-pro'));
              reqSummary.push('stream=' + (!!aiStream && aiOpenAiCompat));
            ...
            try { window.__aiLastMeta = { url: url, model: (payload && payload.model) || (aiModel || ''), stream: !!(aiOpenAiCompat && aiStream) }; } catch (_){ }
            try { window.__aiLastMeta = { url: url, model: (payload && payload.model) || (aiModel || ''), stream: !!(aiOpenAiCompat && aiStream) }; } catch (_) {}
            try { window.__aiRaw = ''; } catch (_) {}

            var startedAt = Date.now();
            var controller = null;
            var lastPieceAt = Date.now();
            var watchdog = null;
            try { controller = new AbortController(); } catch (_){ controller = null; }
            try { window.__aiController = controller; } catch(_){ }
            if (aiOpenAiCompat && aiStream && controller && typeof setInterval==='function') {
              try {
                watchdog = setInterval(function(){ try{ if(Date.now()-lastPieceAt>7000){ try{ aiLog('stream watchdog: timeout -> abort'); }catch(_l){} try{ controller.abort(); }catch(_a){} } }catch(_e){} }, 1000);
              } catch (_werr) {}
            }

            _context2.prev = 12;
            _context2.next = 15;
            return fetch(url, { method: 'POST', headers: headers, body: JSON.stringify(payload), signal: controller ? controller.signal : undefined });
          ...
          case 18:
            ...
            try { if (watchdog) clearInterval(watchdog); } catch(_cw){}
            try { window.__aiController = null; } catch(_cn){}
            try { aiLog('generate fetch error: ' + String((_context2.t0 && _context2.t0.message) || _context2.t0)); } catch(_fl){}
            try { if (aiOpenAiCompat && aiStream) { headers && delete headers['Accept']; } } catch(_hd){}
            try { payload = Object.assign({}, payload, { stream: false }); } catch(_pd){}
            _context2.next = 24;
            try { controller = new AbortController(); } catch(_){ controller = null; }
            try { var __nonStreamTimeout = setTimeout(function(){ try{ if(controller) controller.abort(); }catch(_a){} }, 20000); }catch(_){ }
            return fetch(url, { method: 'POST', headers: headers, body: JSON.stringify(payload), signal: controller ? controller.signal : undefined }).then(function(r){ try{ if(__nonStreamTimeout) clearTimeout(__nonStreamTimeout); }catch(_c){} return r; });
          ...
          case 24:
            ...
            content = '';
            var __uiThrottle = { last: 0, interval: 150 };
              ...
              var pump = function(){
                return reader.read().then(function(r){
                  ...
                  return new Promise(function(resolve){
                    (function step(){
                      var end = Math.min(i + 200, lines.length);
                      for(var j=i;j<end;j++){
                        var s = lines[j].trim();
                        if(!s) continue;
                        ...
                        try{
                          ...
                          if(typeof piece==='string' && piece){
                            content += piece;
                            try{ lastPieceAt = Date.now(); }catch(_){ }
                            try{ window.__aiRaw = (String(window.__aiRaw||'') + piece).slice(-4096); }catch(_){ }
                          }
                        }catch(_e){}
                      }
                      try{ var nowTs = Date.now(); if(nowTs - __uiThrottle.last >= __uiThrottle.interval){ setAiResult(content.slice(-2000)); __uiThrottle.last = nowTs; } }catch(_s){}
                      i = end;
                      if(i < lines.length){ setTimeout(step, 0); } else { resolve(true); }
                    })();
                  }).then(function(){
                    if(!done) return pump();
                    try { if (watchdog) clearInterval(watchdog); } catch(_c){}
                    return true;
                  });
                });
              };
          ...
          case 18:
            ...
            try {
              var cleanedSSE = (function(s){ s=String(s||''); s=s.replace(/^\s*```[a-zA-Z]*\s*/,''); s=s.replace(/\s*```\s*$/,''); var m=s.match(/\{[\s\S]*\}/); return m?m[0]:s; })(content);
              obj = JSON.parse(cleanedSSE);
              if (Array.isArray(obj.message_idx)) {
                var supS = (window.__supportedIds||[]);
                ids = obj.message_idx.map(function(i){ var id=supS[Number(i)]; return typeof id==='string'?id:null; }).filter(Boolean);
              } else if (Array.isArray(obj.message_ids)) {
                ids = obj.message_ids.map(String);
              } else {
                ids = [];
              }
            } catch (_) { ids = []; }

            if (ids && ids.length) { processIds(ids); }
          ...
          case 25:
            ...
            setAiResult(JSON.stringify(data, null, 2).slice(0, 4000));
            try { window.__aiRaw = JSON.stringify(data).slice(-4096); } catch (_){ }

            try { aiLog('response json: ' + JSON.stringify(data).slice(0, 2000)); } catch (_logJson) {}

            if (data && Array.isArray(data.message_idx)) {
              var supD = (window.__supportedIds||[]);
              processIds(data.message_idx.map(function(i){ var id=supD[Number(i)]; return typeof id==='string'?id:null; }).filter(Boolean));
                        } else if (data && data.choices && data.choices[0] && data.choices[0].message && typeof data.choices[0].message.content === 'string') {
              content = data.choices[0].message.content;
              ids = [];
              try {
                var cleanedC = (function(s){ s=String(s||''); s=s.replace(/^\s*```[a-zA-Z]*\s*/,''); s=s.replace(/\s*```\s*$/,''); var m=s.match(/\{[\s\S]*\}/); return m?m[0]:s; })(content);
                obj = JSON.parse(cleanedC);
                if (Array.isArray(obj.message_idx)) {
                  var supO = (window.__supportedIds||[]);
                  ids = obj.message_idx.map(function(i){ var id=supO[Number(i)]; return typeof id==='string'?id:null; }).filter(Boolean);
                } else if (Array.isArray(obj.message_ids)) {
                  ids = obj.message_ids.map(String);
                }
              } catch (_) {}
          ...
          case 24:
            ...
            return res.text().then(function(t){
              ...
              try { setAiResult(String(human).slice(0, 4000)); } catch (_a) {}
          ...
          case 27:
            ...
            try{ if(window.viewerAIState && typeof window.viewerAIState.set==='function'){ window.viewerAIState.set({ aiBusy:false, aiTask:'', aiResult:'' }); } }catch(_){ }
      ...
      (function walk(arr, path) {
        ...
        for (var i = 0; i < arr.length; i++) {
          ...
          if (!hasChildren) {
            ...
            if (key) {
              var enums = Array.isArray(meta && meta.enum) ? meta.enum : (meta && meta.enum_doc ? meta.enum_doc : []);
              var examples = meta && (meta.examples || meta.example) ? (Array.isArray(meta.examples) ? meta.examples : [meta.example]) : [];
              var fmt = meta && meta.format || '';
              var pres = n.presence || 'O';
              var pat = meta && meta.pattern || '';
              var minv = typeof (meta && meta.min) !== 'undefined' ? meta.min : '';
              var maxv = typeof (meta && meta.max) !== 'undefined' ? meta.max : '';
              var range = meta && meta.range || '';
              specMap[key] = { presence: pres, format: fmt, enums: Array.isArray(enums)?enums:[], examples: Array.isArray(examples)?examples:[], pattern: pat, min: minv, max: maxv, range: range };
            }
  ...
  function collectAnalyzeContext() {
    var scenario = '';
    try { scenario = String((window && window.__aiScenario) || aiBuildText || aiPrompt || '').trim(); } catch (_){ }
    var sourceCanvas = Array.isArray(canvasRef && canvasRef.current) ? canvasRef.current : canvas;
    var seq = (Array.isArray(sourceCanvas) ? sourceCanvas : []).map(function(ci){ var m=ci.msg||{}; return String(m.id||''); }).filter(function(x){ return !!x; });
    return { scenario: scenario, message_ids: seq };
  }

  function applyAiSuggestions(sugs) {
    if (!Array.isArray(sugs) || !sugs.length) return;
    function presenceOf(msg, k) {
      var t = (msg && (msg.ie_tree || (msg.info && msg.info.ie_tree))) || [];
      var p = 'O';
      (function walk(arr){ if(!Array.isArray(arr)) return; for(var i=0;i<arr.length;i++){ var n=arr[i]||{}; var key=n.$ref?String(n.$ref).replace(/^#/,''):''; if(key && key===k){ p = n.presence || p; return; } if(Array.isArray(n.children)&&n.children.length) walk(n.children); } })(t);
      return p || 'O';
    }
    function getMeta(msg, k) {
      var d = msg && (msg.ie_dict || (msg.info && msg.info.ie_dict)) || {};
      return d && d[k] || {};
    }
    function isValidOverride(msg, k, v) {
      var val = v===null||typeof v==='undefined'? '': String(v);
      var pres = presenceOf(msg, k);
      if(pres==='M' && val==='') return false;
      var m = getMeta(msg, k);
      var enums = Array.isArray(m && m.enum) ? m.enum : (Array.isArray(m && m.enum_doc) ? m.enum_doc : []);
      if(Array.isArray(enums) && enums.length){ if(val!=='' && enums.indexOf(val)<0) return false; }
      var pat = m && m.pattern || '';
      if(pat){ try { var re = new RegExp(pat); if(val!=='' && !re.test(val)) return false; } catch(_){ } }
      var fmt = m && m.format || '';
      var minv = typeof (m && m.min) !== 'undefined' ? m.min : '';
      var maxv = typeof (m && m.max) !== 'undefined' ? m.max : '';
      var isNumFmt = /^(u?int|number)$/i.test(String(fmt||'')) || (String(fmt||'').indexOf('uint')>=0);
      if(isNumFmt && val!==''){ var num = Number(val); if(!isFinite(num)) return false; if(minv!=='' && Number(num) < Number(minv)) return false; if(maxv!=='' && Number(num) > Number(maxv)) return false; }
      return true;
    }
    setCanvas(function (prev) {
          ...
          Object.keys(s.overrides).forEach(function (k) {
            try {
              if (dict && Object.prototype.hasOwnProperty.call(dict, k)) {
                var v = s.overrides[k];
                if (isValidOverride(ci.msg, k, v)) ie_overrides[k] = v;
              }
            } catch (_) {}
          });
        });
  ...
  result = runLocalConsistencyCheck();
              setAiResult(JSON.stringify(result, null, 2).slice(0, 4000));
            ...
            if (window.showToast) window.showToast('Local consistency check error: ' + (e && e.message || e), 'error');
            } finally {
              setAiBusy(false);
              setAiTask('');
            }
        ...
        while (1) switch (_context4.prev = _context4.next) {
          ...
          case 0:
            ...
            if (aiOpenAiCompat) {
              ...
              msgs = [{ role: 'system', content: sys }, { role: 'user', content: JSON.stringify({ case: caseData, context: collectAnalyzeContext() }) }];
            ...
            try { var __anController = new AbortController(); } catch(_){ __anController = null; }
            try { var __anTimeout = setTimeout(function(){ try{ if(__anController) __anController.abort(); }catch(_a){} }, 20000); }catch(_){ }
            return fetch(url, { method: 'POST', headers: headers, body: JSON.stringify(payload), signal: (__anController ? __anController.signal : undefined) }).then(function(r){ try{ if(__anTimeout) clearTimeout(__anTimeout); }catch(_c){} return r; });
          ...
          case 14:
            ...
            content = '';
            var __anUiThrottle = { last: 0, interval: 150 };

            _context4.next = 19;

            return (function(){ var reader=res.body.getReader(); var td=new TextDecoder('utf-8'); var pump=function(){ return reader.read().then(function(r){ var done=r.done; var val=r.value||new Uint8Array(); var buf=td.decode(val); var lines=buf.split('\n'); var i=0; return new Promise(function(resolve){ (function step(){ var end=Math.min(i+200, lines.length); for(var j=i;j<end;j++){ var s=lines[j].trim(); if(!s) continue; if(s.indexOf('data:')===0) s=s.slice(5).trim(); if(s==='[DONE]') continue; try{ var chunk=JSON.parse(s); var d=chunk&&chunk.choices&&chunk.choices[0]&&(chunk.choices[0].delta||chunk.choices[0].message); var piece=''; if(d&&typeof d.content==='string') piece=d.content; if(!piece&&chunk&&chunk.choices&&chunk.choices[0]&&chunk.choices[0].text) piece=String(chunk.choices[0].text||''); if(piece){ content+=piece; } }catch(_e){} } try{ var nowTs = Date.now(); if(nowTs - __anUiThrottle.last >= __anUiThrottle.interval){ setAiResult(content.slice(-4000)); __anUiThrottle.last = nowTs; } }catch(_s){} try{ if(window.__appendAiLog) window.__appendAiLog(content.slice(-200)); }catch(_l){} i=end; if(i<lines.length){ setTimeout(step,0); } else { resolve(true); } })(); }).then(function(){ if(!done) return pump(); return true; }); }); }; return pump(); })();
          ...
          case 29:
            ...
            return res.text().then(function(t){
              ...
              try { window.__aiRaw = String(t||'').slice(-4096); } catch (_){ }
  ...
  window.viewerAI.reset = function(){
    try{ setAiBusy(false); setAiTask(''); setAiResult(''); }catch(_){}
    try{ if(window.viewerAIState && typeof window.viewerAIState.set==='function'){ window.viewerAIState.set({ aiBusy:false, aiTask:'', aiResult:'' }); } }catch(_){ }
    try{ window.__aiStartAt = null; }catch(_){}
    try{ window.__aiSubset = null; }catch(_){}
    try{ window.__lastAddedUids = []; }catch(_){}
    try{ if(window.__aiController){ window.__aiController.abort(); } }catch(_){}
    try{ window.__aiController = null; }catch(_){}
    try{ if(window.__aiStatusTimer){ clearInterval(window.__aiStatusTimer); window.__aiStatusTimer=null; } }catch(_){}
    try{ if(window.__aiStatusVisible){ window.__aiStatusVisible=false; } }catch(_){}
    try{ if(typeof window.__closeAiStatus==='function') window.__closeAiStatus(); }catch(_){}
  };
  ...
  if (sugs && sugs.length && okIds) applyAiSuggestions(sugs);
      try { setAiResult(JSON.stringify(result, null, 2).slice(0, 4000)); } catch (_) {}
      try { if (window.showToast) window.showToast('Ingested AI result', 'success'); } catch (_e) {}
    } catch (e) {
      if (window.showToast) window.showToast('Ingest AI result error: ' + (e && e.message || e), 'error');
    }
  };
  ...
  window.addEventListener('message', function (ev) {
    try {
      ...
      if (d.type === 'viewerAI.generate') { if (typeof d.prompt === 'string') { updateScenarioText(d.prompt); } return onAiGenerate(d.prompt || null); }
  ...
  (function(){
    try{
      ...
      if(!box){ box=document.createElement('div'); box.id='cfw-ai-status'; box.className='cfw-ai-status hidden'; document.body.appendChild(box); }
      ...
      if(!statusToggle){
        ...
        statusToggle.style.cssText='position:fixed;right:24px;top:8px;z-index:2147483608;background:#1f2937;color:#e5e7eb;border:1px solid #374151;border-radius:9999px;padding:5px 12px;font-size:12px;cursor:pointer;box-shadow:0 6px 18px rgba(0,0,0,0.35)';
        statusToggle.addEventListener('click', function(){ try{ window.__aiStatusUserToggled=true; if(window.openAiStatus) window.openAiStatus(); }catch(_e){} });
        document.body.appendChild(statusToggle);
        try{ window.__aiStatusVisible=false; }catch(_){}
      }
        var append=function(t){ try{ var el=box.querySelector('.log'); if(!el){ box.innerHTML='<h6>AI Status <button class="close" onclick="__closeAiStatus()">脳</button></h6><div class="row"><div>busy</div><div></div></div><div class="row"><div>task</div><div></div></div><div class="row"><div>elapsed</div><div></div></div><div class="row"><div>endpoint</div><div></div></div><div class="row"><div>model</div><div></div></div><div class="row"><div>stream</div><div></div></div><div class="log"></div><div class="raw"><pre style="margin:8px 0;white-space:pre-wrap;word-break:break-word;background:#0b0b0b;border:1px solid #333;padding:6px;border-radius:6px;max-height:160px;overflow:auto"></pre></div>'; el=box.querySelector('.log'); } var cur=el.innerText||''; var next=(cur+String(t||'')).slice(-4000); el.innerHTML='<pre style="margin:0;white-space:pre-wrap;word-break:break-word">'+next+'</pre>'; el.scrollTop=el.scrollHeight; }catch(_){ } };
        var reset=function(t){ try{ box.innerHTML='<h6>AI Status <button class="close" onclick="__closeAiStatus()">脳</button></h6><div class="row"><div>busy</div><div></div></div><div class="row"><div>task</div><div></div></div><div class="row"><div>elapsed</div><div></div></div><div class="row"><div>endpoint</div><div></div></div><div class="row"><div>model</div><div></div></div><div class="row"><div>stream</div><div></div></div><div class="log"><pre style="margin:0;white-space:pre-wrap;word-break:break-word">'+String(t||'')+'</pre></div><div class="raw"><pre style="margin:8px 0;white-space:pre-wrap;word-break:break-word;background:#0b0b0b;border:1px solid #333;padding:6px;border-radius:6px;max-height:160px;overflow:auto"></pre></div>'; }catch(_){ } };
        window.__keepAiPanelOpen = false;
        window.__aiStatusVisible = false;
        window.__closeAiStatus = function(){ try{ window.__aiStatusVisible=false; box.classList.add('hidden'); }catch(_){ } };
        window.openAiStatus = function(){ try{ if(!window.__aiStatusUserToggled) return; window.__aiStatusVisible=true; box.classList.remove('hidden'); }catch(_){ }
          try{
            var baseProbe = String((typeof window!=='undefined' && window.__LLM_API_URL) || (typeof aiApiUrl!=='undefined' && aiApiUrl) || '').trim().replace(/\/+$/,'');
            var modelsUrl = baseProbe ? (/\/v1$/.test(baseProbe) ? baseProbe + '/models' : (baseProbe.replace(/\/chat\/completions$/,'') + '/models')) : '';
            if(modelsUrl){
              (window.__appendAiLog||function(){})('probe '+modelsUrl+'...\n');
              fetch(modelsUrl, { method:'GET' }).then(function(res){
                try{ (window.__appendAiLog||function(){})('probe '+modelsUrl+' -> '+String(res&&res.status)+'\n'); }catch(_){}
                return res.text();
              }).then(function(t){ try{ (window.__appendAiLog||function(){})((String(t||'').slice(0,400))+'\n'); }catch(_){} }).catch(function(e){ try{ (window.__appendAiLog||function(){})('probe error: '+String((e&&e.message)||e)+'\n'); }catch(_){} });
            }
          }catch(_){}
        };
      ...
      try{ if(window.__aiStatusTimer){ clearInterval(window.__aiStatusTimer); } }catch(_){ }
      try{ window.__aiStatusTimer = setInterval(function(){ try{ var stt=(window.viewerAIState&&window.viewerAIState.get&&window.viewerAIState.get())||{}; var busy=!!stt.aiBusy; var task=String(stt.aiTask||''); var res=String(stt.aiResult||''); if(busy && !window.__aiStartAt) window.__aiStartAt=Date.now(); if(!busy && window.__aiStartAt) window.__aiStartAt=null; var elapsed=window.__aiStartAt?Math.floor((Date.now()-window.__aiStartAt)/1000):0; var vis = (typeof window.__aiStatusVisible==='boolean') ? window.__aiStatusVisible : true; box.classList.toggle('hidden', !vis); var rows=box.querySelectorAll('div.row'); if(rows && rows[0]) rows[0].querySelectorAll('div')[1].textContent=String(busy); if(rows && rows[1]) rows[1].querySelectorAll('div')[1].textContent=String(task); if(rows && rows[2]) rows[2].querySelectorAll('div')[1].textContent=String((elapsed||0)+'s'); var meta=window.__aiLastMeta||{}; if(rows && rows[3]) rows[3].querySelectorAll('div')[1].textContent=String(meta.url||''); if(rows && rows[4]) rows[4].querySelectorAll('div')[1].textContent=String(meta.model||''); if(rows && rows[5]) rows[5].querySelectorAll('div')[1].textContent=String(!!meta.stream); var rawBox=box.querySelector('.raw pre'); if(rawBox) rawBox.textContent=String(window.__aiRaw||''); }catch(_){ } }, 1500); }catch(_){ }
    }catch(_){ }
  })();

  (function(){ try{ document.addEventListener('click', function(ev){ try{ var t=ev.target; var btn = (t && t.closest) ? t.closest('button') : null; var label = btn && (btn.textContent||'').trim(); if(!btn || !label) return; if(label==='Generate' || label==='Generate + Optimize'){ try{ if(window.openAiStatus) window.openAiStatus(); }catch(_e){} try{ if(window.__appendAiLog) window.__appendAiLog('[btn-click] '+label+'\n'); }catch(_l){} var txt=''; try{ var ta=document.querySelector('.cfw-ai-text'); txt = ta && ta.value || ''; }catch(_t){} try{ window.postMessage({type:'viewerAI.generate', prompt: txt}, '*'); }catch(_p){} try{ if(typeof onAiGenerate==='function') onAiGenerate(txt||null); }catch(_g){} } }catch(_){ } }, true); }catch(_){ } })();
  ...
  function onAiBuildFlow() {
    try {
      if (insecureWorker) { try{ if(window.showToast) window.showToast('Browser blocked HTTP worker from HTTPS page. Open the viewer locally or use an HTTPS worker.', 'error'); }catch(_e){} return; }
      
      try { if (window.openAiStatus) window.openAiStatus(); } catch (_) {}
      setAiBusy(true); setAiTask('generate'); setAiResult('');
      try{ window.__aiStartAt = Date.now(); }catch(_){}
      ...
      url = /\/v1$/.test(base) ? base + '/chat/completions' : base;
      var SYSTEM_PROMPT_STRICT_JSON = 'Output a single strict JSON object {"message_ids":[...]} ONLY. Use full truth IDs strictly from supported_message_ids; maintain UE/network directionality; keep chronological order. Aim for end-to-end coverage: include as many spec-recognized messages across involved nodes as applicable; include request/response legs; include optional normative steps when typically required; duplicates ALLOWED when normative flow repeats; NEVER invent or abbreviate IDs; NO explanations/titles/markdown/code fences. If none apply, return {"message_ids":[]}.';
      var sys = SYSTEM_PROMPT_STRICT_JSON;
      var allIds = (Array.isArray(truth)?truth:[]).map(function(m){ return m && m.id; }).filter(Boolean);
      var filtered = Array.from(new Set(allIds));
      var msgs = [ { role:'system', content: sys }, { role:'user', content: String(aiBuildText||aiPrompt||'') } ];
      if (filtered.length) msgs.push({ role:'user', content: 'supported_message_ids: ' + JSON.stringify(filtered) });
      var payload = { model: String(aiOpenAiCompat ? (aiModel||'gemini-2.5-pro') : (aiModel||'gemini-2.5-pro')), messages: msgs, response_format: { type: 'json_object' } };
      if (aiOpenAiCompat) payload = Object.assign({}, payload, { stream: !!aiStream });
      if (aiOpenAiCompat && aiStream) headers['Accept'] = 'text/event-stream';
      try{ aiResetLog('AI Build Generate'); aiLog('POST '+url+' (openai='+aiOpenAiCompat+', stream='+(!!aiStream&&aiOpenAiCompat)+')'); aiLog('payload='+JSON.stringify(payload).slice(0,1500)); }catch(_){}
      var __controller=null, __watchdog=null, __lastPieceAt=Date.now();
      try{ __controller = new AbortController(); }catch(_){ __controller=null; }
      if (aiOpenAiCompat && aiStream && __controller && typeof setInterval==='function') {
        try{ __watchdog = setInterval(function(){ try{ if(Date.now()-__lastPieceAt>7000){ try{ aiLog('stream watchdog: timeout -> abort'); }catch(_l){} try{ __controller.abort(); }catch(_a){} } }catch(_e){} }, 1000); }catch(_w){}
      }
      fetch(url, { method:'POST', headers: headers, body: JSON.stringify(payload), signal: (__controller ? __controller.signal : undefined) }).then(function(res){
        if (!(aiOpenAiCompat && aiStream && res && res.body)) {
          if(!res.ok){
            return res.json().catch(function(){ return {}; }).then(function(j){
              try{ setAiResult(JSON.stringify(j||{}, null, 2)); }catch(_){ }
              try{ if(window.showToast) window.showToast('Generate failed: HTTP '+(res&&res.status),'error'); }catch(_e){}
              return null;
            });
          }
          return res.json();
        ...
        var pump = function(){
          return reader.read().then(function(r){
            ...
            for(var i=0;i<lines.length;i++){
              ...
              if(s==='[DONE]') continue;
              try{
                try{ if(window.__appendAiLog) window.__appendAiLog(s+'\n'); }catch(_log){}
                var chunk=JSON.parse(s);
                var d = chunk && chunk.choices && chunk.choices[0] && (chunk.choices[0].delta || chunk.choices[0].message);
                var piece = '';
                if(d && typeof d.content==='string') piece = d.content;
                ...
                if(piece){
                  content += piece;
                  try{ setAiResult(content.slice(-4000)); }catch(_){}
                  try{ if(window.__appendAiLog) window.__appendAiLog(piece); }catch(_l){}
                  try{ __lastPieceAt = Date.now(); }catch(_t){}
                }
              }catch(_e){}
            }
            if(!done) return pump();
            try{ if(__watchdog) clearInterval(__watchdog); }catch(_c){}
        ...
        try{
          if(!data) return;
          var ids=[];
          if(typeof data.streamed==='string'){
            setAiResult(data.streamed.slice(-4000));
            try{ var o = JSON.parse(data.streamed); if(Array.isArray(o.message_idx)){ var sup=(window.__supportedIds||[]); ids = o.message_idx.map(function(i){ var id=sup[Number(i)]; return typeof id==='string'?id:null; }).filter(Boolean); } }catch(_e){}
          } else {
            setAiResult(JSON.stringify(data,null,2));
            if(data && data.error){ /* do not parse ids on error */ }
            else if(Array.isArray(data.message_idx)){ var sup=(window.__supportedIds||[]); ids = data.message_idx.map(function(i){ var id=sup[Number(i)]; return typeof id==='string'?id:null; }).filter(Boolean); }
            else if(data && data.candidates && data.candidates[0] && data.candidates[0].content){
              try{
                var parts = data.candidates[0].content.parts||[];
                var text='';
                for(var pi=0;pi<parts.length;pi++){ if(parts[pi] && typeof parts[pi].text==='string') text += parts[pi].text; }
                if(text){
                  var cleaned = (function(s){ s=String(s||''); s=s.replace(/^\s*```[a-zA-Z]*\s*/,''); s=s.replace(/\s*```\s*$/,''); var m=s.match(/\{[\s\S]*\}/); return m?m[0]:s; })(text);
                  var objg=JSON.parse(cleaned);
                  if(Array.isArray(objg.message_idx)){
                    var sup2=(window.__supportedIds||[]);
                    ids = objg.message_idx.map(function(i){ var id=sup2[Number(i)]; return typeof id==='string'?id:null; }).filter(Boolean);
                  }
                }
              }catch(_ge){}
            }
            else if(data && data.choices && data.choices[0] && data.choices[0].message && typeof data.choices[0].message.content==='string'){
              var cc = data.choices[0].message.content;
              try{
                var cleanedC = (function(s){ s=String(s||''); s=s.replace(/^\s*```[a-zA-Z]*\s*/,''); s=s.replace(/\s*```\s*$/,''); var m=s.match(/\{[\s\S]*\}/); return m?m[0]:s; })(cc);
                var obj=JSON.parse(cleanedC);
                if(Array.isArray(obj.message_idx)){
                  var sup3=(window.__supportedIds||[]);
                  ids = obj.message_idx.map(function(i){ var id=sup3[Number(i)]; return typeof id==='string'?id:null; }).filter(Boolean);
                }
              }catch(_e){}
            }
          }
          if(ids && ids.length){
            try{
              var valid = [];
              try{ var validSet = new Set((Array.isArray(truth)?truth:[]).map(function(m){ return m && m.id; }).filter(Boolean)); valid = ids.filter(function(x){ return validSet.has(x); }); }catch(_vf){ valid = ids.slice(); }
              if(valid.length){ buildCaseFromIds(valid); try{ showAiStatus('Parsed '+valid.length+' ids'); }catch(_s){} }
              else { try{ showAiStatus('Parsed 0 valid ids'); }catch(_s2){} }
            }catch(_b){}
            if (window.aiAutoOptimize) {
              try {
                var u = Array.isArray(window.__lastAddedUids)?window.__lastAddedUids.slice():[];
                if(u.length){ try{ window.__aiSubset = null; }catch(_n){} onAiAnalyzeCase(); }
              } catch(_){ }
            }
            try{ setAiBusy(false); setAiTask(''); }catch(_){ }
          }
        }catch(_e){}
      }).catch(function(e){
        try{ if(__watchdog) clearInterval(__watchdog); }catch(_cw){}
        try{ aiLog('build fetch error: '+String((e&&e.message)||e)); }catch(_l){}
        if (aiOpenAiCompat && aiStream) {
          try{ if(headers) delete headers['Accept']; }catch(_hd){}
          try{ payload = Object.assign({}, payload, { stream: false }); }catch(_pd){}
          try{ var __nsCtrl = new AbortController(); }catch(_){ __nsCtrl = null; }
          try{ var __nsTimeout = setTimeout(function(){ try{ if(__nsCtrl) __nsCtrl.abort(); }catch(_a){} }, 20000); }catch(_){ }
          return fetch(url, { method:'POST', headers: headers, body: JSON.stringify(payload), signal: (__nsCtrl ? __nsCtrl.signal : undefined) }).then(function(r){ return r.ok ? r.json() : r.text(); }).then(function(x){ try{ setAiResult(typeof x==='string'? String(x).slice(0,4000) : JSON.stringify(x,null,2).slice(0,4000)); }catch(_s){} return x; }).finally(function(){ try{ if(__nsTimeout) clearTimeout(__nsTimeout); }catch(_c){} });
        } else {
          var errText = 'AI generation request error: ' + (e && e.message || e) + ' (endpoint: ' + String(aiApiUrl || 'n/a') + ')'; try { if (window.showToast) window.showToast(errText, 'error'); } catch (_) {} try{ setAiResult(errText); }catch(_){}
        }
      }).finally(function(){ setAiBusy(false); setAiTask(''); try{ if(!window.__keepAiPanelOpen) setAiBuildOpen(false); }catch(_){ } try{ window.__aiStartAt=null; }catch(_){ } try{ window.__aiController=null; }catch(_){ } });
      try{ if(window.viewerAIState && typeof window.viewerAIState.set==='function'){ window.viewerAIState.set({ aiBusy:false, aiTask:'', aiResult:'' }); } }catch(_){ }
      try{ setTimeout(function(){ try{ setAiBusy(false); setAiTask(''); }catch(_){} }, 10000); }catch(_){}
    } catch (e) { if (window.showToast) window.showToast('AI generation request error: ' + (e && e.message || e), 'error'); }
  }
  ...
  React.useEffect(function(){
    try{
      ...
      btnGen.addEventListener('click', function(){ try{ var txt=ta.value; updateScenarioText(txt); try{ if(window.openAiStatus) window.openAiStatus(); }catch(_){} try{ if(window.__appendAiLog) window.__appendAiLog('btnGen clicked\n'); }catch(_){} try{ window.postMessage({type:'viewerAI.generate', prompt: txt}, '*'); }catch(_){} try{ onAiGenerate(txt); }catch(_){ } }catch(_){ } });
      btnGenOpt.addEventListener('click', function(){ try{ var txt=ta.value; updateScenarioText(txt); window.aiAutoOptimize = true; try{ if(window.openAiStatus) window.openAiStatus(); }catch(_){} try{ if(window.__appendAiLog) window.__appendAiLog('btnGenOpt clicked\n'); }catch(_){} try{ window.postMessage({type:'viewerAI.generate', prompt: txt}, '*'); }catch(_){} try{ onAiGenerate(txt); }catch(_){ } }catch(_){ } });
  ...
  // ----- Case 锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷 localStorage锟斤拷 -----
  ...
  function openCaseByName(name) {
    ...
    // 默锟斤拷锟斤拷锟斤拷锟阶次达拷 Case 时锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷息锟斤拷锟斤拷为锟桔碉拷
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "text-sm font-semibold text-zinc-100 flex items-center gap-3"
  }, /*#__PURE__*/React.createElement("div", {
    className: "p-2 rounded-lg bg-pink-500/20 border border-pink-400/30"
  }, /*#__PURE__*/React.createElement(Icon, {
    name: "ai",
    size: 16,
    className: "text-pink-400"
  })), "AI Settings & Input"), /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-12 gap-3"
  }, /*#__PURE__*/React.createElement("div", {
    className: "col-span-6"
  }, /*#__PURE__*/React.createElement("input", {
    className: "bg-zinc-900/80 border border-zinc-600/50 rounded-lg px-4 py-3 text-sm w-full",
    placeholder: "API Endpoint",
    value: aiApiUrl,
    onChange: function onChange(e) {
      return setAiApiUrl(e.target.value);
    }
  })), /*#__PURE__*/React.createElement("div", {
    ...
    value: aiApiKey,
    onChange: function onChange(e) {
      return setAiApiKey(e.target.value);
    }
  })), /*#__PURE__*/React.createElement("div", {
    className: "col-span-3"
  }, /*#__PURE__*/React.createElement("input", {
    ...
    value: aiModel,
    onChange: function onChange(e) {
      return setAiModel(e.target.value);
    }
  }))), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between"
    ...
    type: "checkbox",
    checked: aiOpenAiCompat,
    onChange: function onChange(e) {
      return setAiOpenAiCompat(e.target.checked);
    }
  }), "OpenAI Compatible Mode"), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-2"
  }, /*#__PURE__*/React.createElement(IconButton, {
    icon: "refresh",
    title: "List Models",
    onClick: onAiListModels,
    disabled: !aiApiUrl
  }))), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between"
  }, /*#__PURE__*/React.createElement("label", {
    ...
    type: "checkbox",
    checked: aiStream,
    onChange: function onChange(e) { return setAiStream(e.target.checked); }
  }), "Streaming (chat.completions)")), /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-zinc-400 mb-2"
  }, "Default model: gemini-2.5-pro. Adjust the model name above if you need a different backend."), /*#__PURE__*/React.createElement("textarea", {
    className: "bg-zinc-900/80 border border-zinc-600/50 rounded-lg px-4 py-3 text-sm font-mono min-h-[160px] w-full resize-none",
    placeholder: "Example: give me a user initial registration signaling flow (can use current imported messages)",
    value: aiPrompt,
    onChange: function onChange(e) {
    ...
    disabled: aiBusy || insecureWorker
  }), /*#__PURE__*/React.createElement(IconButton, {
    icon: "ai",
    title: "AI Analyze & Optimize",
    onClick: onAiAnalyzeCase,
    disabled: aiBusy || insecureWorker
  }), /*#__PURE__*/React.createElement(IconButton, {
    icon: "check",
    title: "Local Consistency Check",
    onClick: onLocalAnalyzeCase,
    disabled: aiBusy
  }), aiBusy && /*#__PURE__*/React.createElement("span", {
    className: "text-xs text-zinc-400"
  }, aiTask === 'analyze' ? 'Analyzing...' : 'Generating...')), /*#__PURE__*/React.createElement("div", {
    className: "space-y-2"
  }, insecureWorker && /*#__PURE__*/React.createElement("div", {
    className: "text-xs text-red-400 bg-red-500/10 border border-red-500/40 rounded-lg px-3 py-2"
  }, "This page is served over HTTPS but the configured worker endpoint uses HTTP. Browsers block those requests. Run the viewer locally over HTTP or expose the worker over HTTPS."), /*#__PURE__*/React.createElement("div", {
    className: "text-xs text-zinc-400"
  }, "Response preview/diagnostics:"), /*#__PURE__*/React.createElement("textarea", {
    readOnly: true,
    className: "bg-zinc-900/80 border border-zinc-600/50 rounded-lg px-4 py-3 text-xs font-mono min-h-[120px] w-full resize-none",
    value: aiResult
  })))), /*#__PURE__*/React.createElement("div", {
    className: "col-span-5 space-y-4",
    id: "flow-canvas-root"
    ...
    className: "bg-white/5 border border-white/10 rounded-xl p-6 space-y-4"
  }, /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", { className: "flex items-center gap-2 mb-3" }, /*#__PURE__*/React.createElement(IconButton, { icon: "generate", title: "AI Generate Case", onClick: notifyUseAiBuild, disabled: true }), /*#__PURE__*/React.createElement(IconButton, { icon: "ai", title: "AI Improve IE (Analyze)", onClick: notifyUseAiBuild, disabled: true })), /*#__PURE__*/React.createElement("div", {
    className: "font-medium text-zinc-100"
  }, "Create case from message id sequence")), /*#__PURE__*/React.createElement("textarea", {
    className: "bg-zinc-900/80 border border-zinc-600/50 rounded-lg px-4 py-3 text-sm font-mono min-h-[120px] w-full resize-none",
    placeholder: "Enter IDs in order, comma/space separated, e.g., msg_17310 msg_17311 msg_17312",
    value: idsText,
    onChange: function onChange(e) {
      return setIdsText(e.target.value);
    }
  }), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-2"
  }, /*#__PURE__*/React.createElement(IconButton, {
    icon: "plus",
    title: "Create case from IDs",
    onClick: createCaseByIdsFromText,
    ...
    className: "p-2 rounded-lg bg-green-500/20 border border-green-400/30"
  }, /*#__PURE__*/React.createElement(Icon, {
    name: "settings",
    size: 16,
    className: "text-green-400"
  })), "Input Source (JSONL / JSON)"), /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-12 gap-3"
  }, /*#__PURE__*/React.createElement("div", {
    ...
    className: "bg-zinc-900/80 border border-zinc-600/50 rounded-lg px-4 py-3 text-sm font-mono min-h-[180px] w-full resize-none",
    ...
    className: "text-xs text-zinc-400"
  }, "Tip: one JSON object per line, enclosed in curly braces."))))), /*#__PURE__*/React.createElement("div", {
    className: "col-span-5 space-y-4"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bg-white/5 border border-white/10 rounded-xl p-6 space-y-4"
  }, /*#__PURE__*/React.createElement("div", {
    className: "font-medium text-zinc-100"
  }, "Case Import / Export"), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-2"
  }, /*#__PURE__*/React.createElement(IconButton, {
    icon: "export",
    title: "Export Case",
    onClick: exportCaseSelected
  }), /*#__PURE__*/React.createElement(IconButton, {
    icon: "import",
    title: "Import Case",
    onClick: importCase
    ...
    className: "text-xs text-zinc-400"
  }, "Import/Export the current canvas case (if a card is selected, only export that card)")), /*#__PURE__*/React.createElement("div", {
    className: "bg-white/5 border border-white/10 rounded-xl p-6 space-y-4"
  }, /*#__PURE__*/React.createElement("div", {
    className: "font-medium text-zinc-100"
  }, "Viewer Settings"), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-sm text-zinc-300"
  }, "Default Global Edit"), /*#__PURE__*/React.createElement("input", {
    type: "checkbox",
    checked: globalEditMode,
    onChange: function onChange(e) {
      return setGlobalEditMode(e.target.checked);
    }
  })), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-sm text-zinc-300"
  }, "Default Compact Mode"), /*#__PURE__*/React.createElement("input", {
    type: "checkbox",
    checked: compactMode,
    onChange: function onChange(e) {
      return setCompactMode(e.target.checked);
    }
  })), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-sm text-zinc-300"
  }, "Show instance roles by default"), /*#__PURE__*/React.createElement("input", {
    type: "checkbox",
    checked: inst,
    onChange: function onChange(e) {
      return setInst(e.target.checked);
    ...
    type: "number",
    className: "bg-zinc-900/80 border border-zinc-600/50 rounded-lg px-3 py-2 text-sm w-full",
    value: itemsPerPage,
    onChange: function onChange(e) {
      return setItemsPerPage(Math.max(1, Number(e.target.value) || 1));
    }
  })), /*#__PURE__*/React.createElement("div", {
    className: "text-xs text-zinc-400"
  }, "Changes are auto-saved to browser localStorage and take effect immediately."), /*#__PURE__*/React.createElement("div", {
    className: "fixed right-6 bottom-6 w-[360px] bg-zinc-950/90 border border-white/10 rounded-2xl shadow-2xl p-4 text-xs text-zinc-200 space-y-2",
    style: { zIndex: 2147483606 }
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between gap-3 text-[11px] uppercase tracking-wide text-zinc-400"
  }, /*#__PURE__*/React.createElement("span", null, "Busy: ", aiBusy ? 'Yes' : 'No'), /*#__PURE__*/React.createElement("span", null, "Task: ", aiTask || 'idle'), /*#__PURE__*/React.createElement("span", null, "Elapsed: ", aiElapsed, "s")), /*#__PURE__*/React.createElement("div", {
    className: "max-h-48 overflow-auto rounded-xl bg-black/20 border border-white/10 font-mono text-[11px] leading-snug whitespace-pre-wrap px-2 py-2 text-zinc-300"
  }, aiInlineLog || 'Idle - awaiting AI tasks')))))));
}
function BuildPane(props) {
  return BuildPane_old(props);
}
function CanvasPane(props) {
  return CanvasPane_old(props);
}
function CanvasCard(_ref5) {
  var ci = _ref5.ci,
    idx = _ref5.idx,
    ...
    inst = _ref5.inst,
    rm = _ref5.rm,
    onDragStartRight = _ref5.onDragStartRight,
    _ref5$compactMode = _ref5.compactMode,
    compactMode = _ref5$compactMode === void 0 ? false : _ref5$compactMode,
    onUpdateMessage = _ref5.onUpdateMessage,
    _ref5$globalEditMode = _ref5.globalEditMode,
    globalEditMode = _ref5$globalEditMode === void 0 ? false : _ref5$globalEditMode;
  var m = ci.msg;
  var _React$useState59 = React.useState(Boolean(ci && ci.expanded)),
    _React$useState60 = _slicedToArray(_React$useState59, 2),
    open = _React$useState60[0],
    setOpen = _React$useState60[1];
  React.useEffect(function () {
    if (globalEditMode) {
      setOpen(true);
    } else {
      // 锟斤拷锟截憋拷全锟街编辑时锟斤拷锟斤拷锟剿碉拷锟斤拷片锟斤拷默锟斤拷展锟斤拷状态锟斤拷ci.expanded 锟斤拷锟桔碉拷锟斤拷
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "card " + (selected ? 'selected' : '') + (compactMode ? ' compact' : ''),
    style: {
      position: 'relative',
      overflow: 'visible',
      zIndex: 'auto'
    ...
    className: (compactMode ? "px-2 py-1" : "px-3 py-2") + " flex items-center justify-between"
  }, /*#__PURE__*/React.createElement("div", {
    className: "handle",
    draggable: true,
    onDragStart: onDragStartRight,
    role: "button",
    "aria-label": "CardHandle " + idx,
    "data-idx": String(idx),
    onClick: function onClick() {
      try {
        window.__dragFromUid = ci.uid;
      } catch (_) {}
    }
  }, /*#__PURE__*/React.createElement("span", {
    className: "grip",
...
function CardBody(_ref6) {
  var _editedMsg$doc, _editedMsg$doc2;
  var ci = _ref6.ci,
    inst = _ref6.inst,
    _ref6$compactMode = _ref6.compactMode,
    compactMode = _ref6$compactMode === void 0 ? false : _ref6$compactMode,
    onUpdateMessage = _ref6.onUpdateMessage,
    _ref6$globalEditMode = _ref6.globalEditMode,
    globalEditMode = _ref6$globalEditMode === void 0 ? false : _ref6$globalEditMode;
  var m = ci.msg;
  var _React$useState61 = React.useState(Boolean(ci && ci.forceEdit) || Boolean(globalEditMode)),
    _React$useState62 = _slicedToArray(_React$useState61, 2),
    isEditing = _React$useState62[0],
    setIsEditing = _React$useState62[1];
  React.useEffect(function () {
    setIsEditing(Boolean(ci && ci.forceEdit) || Boolean(globalEditMode));
  }, [globalEditMode, ci]);
  var _React$useState63 = React.useState(_objectSpread({}, m)),
    _React$useState64 = _slicedToArray(_React$useState63, 2),
    editedMsg = _React$useState64[0],
    setEditedMsg = _React$useState64[1];
  var _React$useState65 = React.useState(JSON.stringify(m, null, 2)),
    _React$useState66 = _slicedToArray(_React$useState65, 2),
    rawText = _React$useState66[0],
    setRawText = _React$useState66[1];
  var _React$useState67 = React.useState(''),
    _React$useState68 = _slicedToArray(_React$useState67, 2),
    jsonError = _React$useState68[0],
    setJsonError = _React$useState68[1];

  // - 
  var DEBUG_FORCE_SHOW_EDIT = false;
  var summary = m.doc && m.doc.summary_en ? m.doc.summary_en : '(none)';
  var commentMd = m.doc && m.doc.comment_md ? m.doc.comment_md : '';
  var handleSave = function handleSave() {
    var updatedMsg = _objectSpread({}, editedMsg);
    ci.msg = updatedMsg; // 
    if (onUpdateMessage) {
  ...
  var validateRaw = function validateRaw() {
    try {
      var parsed = parseJSONAny(rawText);
      if (parsed.items && parsed.items.length === 1) {
        setJsonError('');
        if (window.showToast) window.showToast('JSON validation passed', 'success');
      } else {
        setJsonError('Please provide a valid message object (found ' + (parsed.items ? parsed.items.length : 0) + ')');
        if (window.showToast) window.showToast('JSON validation failed', 'warning');
      }
    } catch (e) {
      setJsonError('JSON validation error: ' + (e && e.message || e));
    }
  };
  var applyRaw = function applyRaw() {
    try {
      var parsed = parseJSONAny(rawText);
      if (parsed.items && parsed.items.length === 1) {
        var updatedMsg = parsed.items[0];
        setEditedMsg(_objectSpread({}, updatedMsg));
        setJsonError('');
      ...
      setJsonError('JSON apply error: ' + (e && e.message || e));
    }
  };
  var syncRawFromForm = function syncRawFromForm() {
    try {
      setRawText(JSON.stringify(editedMsg, null, 2));
      if (window.showToast) window.showToast('Synced form to JSON', 'info');
    } catch (e) {}
  };

  // 锟街段改讹拷统一锟斤拷锟斤拷锟斤拷Global Edit 锟斤拷锟斤拷时锟斤拷锟侥讹拷锟斤拷时写锟截诧拷锟斤拷锟斤拷 onUpdateMessage
  var handleFieldChange = function handleFieldChange(patch) {
    var updated = _objectSpread(_objectSpread({}, editedMsg), patch);
  ...
  if (compactMode) {
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "text-xs text-zinc-400"
    }, "[compact:true] [editing:", String(isEditing), "]"), /*#__PURE__*/React.createElement("div", {
    ...
    }, m.name || 'N/A')), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      className: "text-zinc-400 text-xs"
      ...
      className: "font-medium text-xs"
    }, m.release))), inst && /*#__PURE__*/React.createElement("div", {
      className: "grid grid-cols-2 gap-2"
    }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      className: "text-xs text-zinc-400"
    }, "From"), /*#__PURE__*/React.createElement(RoleSelectRestricted, {
      ci: ci,
      side: "from"
    })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      className: "text-xs text-zinc-400"
    }, "To"), /*#__PURE__*/React.createElement(RoleSelectRestricted, {
      ci: ci,
      side: "to"
    }))), !globalEditMode && /*#__PURE__*/React.createElement("div", {
      className: "flex gap-2 mt-2"
    }, !isEditing ? /*#__PURE__*/React.createElement(IconButton, {
      icon: "pencil",
      size: "sm",
      title: "Edit",
      onClick: function onClick() {
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, m.name || 'Message'), /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'relative',
      zIndex: '1002'
    }
  }, globalEditMode ? /*#__PURE__*/React.createElement("span", {
    className: "text-xs text-zinc-400"
  }, "Global Edit") : !isEditing ? /*#__PURE__*/React.createElement(IconButton, {
    icon: "pencil",
    size: "sm",
    title: "Edit",
    onClick: function onClick() {
    ...
    value: ((_editedMsg$doc = editedMsg.doc) === null || _editedMsg$doc === void 0 ? void 0 : _editedMsg$doc.summary_en) || '',
    onChange: function onChange(e) {
      return handleFieldChange({
    ...
    value: ((_editedMsg$doc2 = editedMsg.doc) === null || _editedMsg$doc2 === void 0 ? void 0 : _editedMsg$doc2.comment_md) || '',
    onChange: function onChange(e) {
      return handleFieldChange({
        doc: _objectSpread(_objectSpread({}, editedMsg.doc || {}), {}, {
    ...
    className: "leading-relaxed whitespace-pre-wrap bg-zinc-900 border border-zinc-700 rounded-md px-3 py-2 text-sm text-zinc-200 min-h-[100px] w-full"
  }, commentMd))), inst && /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-2 gap-3"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    ...
    className: "border border-zinc-800 rounded-lg bg-zinc-900/30 p-3 space-y-3"
  }, /*#__PURE__*/React.createElement("div", {
    ...
    className: "border border-zinc-800 rounded-lg bg-zinc-900/30 p-3 space-y-2"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between"
  }, /*#__PURE__*/React.createElement("div", {
    className: "font-medium text-zinc-200 flex items-center gap-2"
  ...
  })), "Raw JSON"), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-2"
  }, /*#__PURE__*/React.createElement("button", {
    className: "btn btn-sm",
    onClick: syncRawFromForm
  }, "Sync from form"), /*#__PURE__*/React.createElement("button", {
    className: "btn btn-sm",
    onClick: validateRaw
  }, "Validate"), /*#__PURE__*/React.createElement("button", {
    className: "btn btn-sm btn-primary",
    onClick: applyRaw
  }, "Apply JSON"))), /*#__PURE__*/React.createElement("textarea", {
    className: "bg-zinc-900 border border-zinc-700 rounded-md px-3 py-2 text-sm font-mono text-zinc-300 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent min-h-[180px] w-full resize-none",
    value: rawText,
    onChange: function onChange(e) {
      return setRawText(e.target.value);
    }
  }), jsonError && /*#__PURE__*/React.createElement("div", {
...
function buildPreviewHTML(lanesArr, messagesArr) {
  ...
  parts.push(' #legend{position:absolute;top:6px;left:6px;background:#18181b;border:1px solid #27272a;border-radius:8px;padding:6px 8px;color:#f2f2f2;font-size:12px;opacity:0.92}');
  parts.push(' #legend .item{display:flex;align-items:center;gap:6px;margin:2px 0}');
  parts.push(' #legend .swatch{width:12px;height:12px;border-radius:2px;display:inline-block}');
  ...
  parts.push(' var legend=document.createElement("div"); legend.id="legend"; legend.innerHTML = "<div class=\"item\"><span class=\"swatch\" style=\"background:#2563eb\"></span><span>N2/NGAP</span></div><div class=\"item\"><span class=\"swatch\" style=\"background:#7f1d1d\"></span><span>N1/NAS</span></div><div class=\"item\"><span class=\"swatch\" style=\"background:#7f00ff\"></span><span>N4/PFCP</span></div><div class=\"item\"><span class=\"swatch\" style=\"background:#0f766e\"></span><span>Nn/SBI</span></div><div class=\"item\"><span class=\"swatch\" style=\"background:#b45309\"></span><span>SGs/SGsAP</span></div><div class=\"item\"><span class=\"swatch\" style=\"background:#0ea5e9\"></span><span>S1/S1AP</span></div>"; diagramContainer.appendChild(legend);');
  parts.push(" function colorOf(iface, proto){ var k=String((iface||proto||\"\")).toUpperCase(); if(k==='N2' || k==='NGAP') return '#2563eb'; if(k==='N1' || k==='NAS' || k==='NAS-5G') return '#7f1d1d'; if(k==='N4' || k==='PFCP') return '#7f00ff'; if(k==='NN' || k==='HTTP/SBI' || k==='SBI' || k==='HTTP') return '#0f766e'; if(k==='SGS' || k==='SGSAP') return '#b45309'; if(k==='S1' || k==='S1AP') return '#0ea5e9'; return '#000'; }");
  ...
  parts.push(' var curPath = path ? (path + "." + seg) : seg;');
  parts.push(' out.push({path:curPath, presence:n.presence||"O", key:key, meta:meta, node:n});');
  ...
  parts.push(' var hdr=(m.info&&m.info.header)||{};');
  parts.push(' var stroke=colorOf(hdr["interface"], hdr.protocol);');
  parts.push(' ln.setAttribute("stroke", stroke); ln.setAttribute("marker-end","url(#arrow)"); svg.appendChild(ln);');
  ...
  parts.push(' if(info.http && info.http.sync_response){ var sr=info.http.sync_response; var opts=sr.options||[]; show.http_sync_response = { default: sr.default, options: opts.map(function(o){ return { code:o.code, family:o.family, has_body:o.has_body, content_type:o.content_type, schema_ref:o.schema_ref||"" }; }) }; }');
  parts.push(' if(info.status!==undefined) show.status = info.status;');
  parts.push(' if(info.body!==undefined) show.body = info.body;');
  parts.push(' }else{');
  parts.push(' var hdrMerged = (info.header || m.header || m.headers || {});');
  parts.push(' var payloadMerged = (info.payload || info.payload_kv || info.fields || m.payload || m.payload_kv || m.fields || info.ies || m.ies || {});');
  parts.push(' if(!payloadMerged || !Object.keys(payloadMerged).length){ var flat=[]; flattenIE(tree, dict, "", flat); var p={}; for(var i=0;i<flat.length;i++){ var f=flat[i]; var key=f.key; var meta=f.meta||{}; var node=f.node||{}; var ov = (overrides && hasOwn(overrides,key)) ? overrides[key] : ""; var defv = defaultOf(node, meta); var v = ov!=="" ? ov : defv; if(v!=="" || (typeof v!=="undefined")){ p[f.path] = String(v); } } payloadMerged = p; }');
  parts.push(' if(hdrMerged && Object.keys(hdrMerged).length) show.header = hdrMerged;');
  parts.push(' // do not render payload via generic walker; we will render IE structure inside a custom payload block');
  parts.push(' if(info.primary_ref) show.primary_ref = info.primary_ref;');
  parts.push(' }');
  parts.push(' jsonViewer.innerHTML="";');
  parts.push(' var h=document.createElement("div"); h.style.fontWeight="600"; h.style.marginBottom="6px";');
  parts.push(' var hdr=(m.info&&m.info.header)||{};');
  parts.push(' var fromTxt=(m.from_role||m.from)||"";');
  parts.push(' var toTxt=(m.to_role||m.to)||"";');
  parts.push(' var msgTxt=hdr.msg||"";');
  parts.push(' h.textContent=(fromTxt+" \u2192 "+toTxt+" "+msgTxt);');
  parts.push(' var makeBadge=function(txt){ var b=document.createElement("span"); b.className="badge"; b.textContent=String(txt||""); return b; };');
  parts.push(' if(hdr.protocol) h.appendChild(makeBadge(hdr.protocol));');
  parts.push(' if(hdr.interface) h.appendChild(makeBadge(hdr.interface));');
  parts.push(' if(hdr.release) h.appendChild(makeBadge(hdr.release));');
  parts.push(' if(hdr.id) h.appendChild(makeBadge(hdr.id));');
  parts.push(' if(m.family) h.appendChild(makeBadge(m.family));');
  parts.push(' jsonViewer.appendChild(h);');
  parts.push(' renderJSON(show, jsonViewer);');
  parts.push(' var payloadBlock=document.createElement("details"); payloadBlock.open=true; var payloadSummary=document.createElement("summary"); payloadSummary.textContent="payload"; payloadBlock.appendChild(payloadSummary);');
  parts.push(' var holder=document.createElement("div"); payloadBlock.appendChild(holder);');
  parts.push(' jsonViewer.appendChild(payloadBlock);');
  ...
  parts.push(' var primaryRef = (m.primary_ref || (m.info && m.info.primary_ref) || null);');
  parts.push(' var extraRefs = [];');
  parts.push(' if(Array.isArray(m.spec_refs)) extraRefs = extraRefs.concat(m.spec_refs);');
  parts.push(' if(m.info && Array.isArray(m.info.spec_refs)) extraRefs = extraRefs.concat(m.info.spec_refs);');
  parts.push(' if(docRef && Array.isArray(docRef.refs)) extraRefs = extraRefs.concat(docRef.refs);');
  parts.push(' if(docRef && Array.isArray(docRef.spec_refs)) extraRefs = extraRefs.concat(docRef.spec_refs);');
  parts.push(' var combined = [];');
  parts.push(' if(summaryText) combined.push("## Summary\\n\\n" + String(summaryText));');
  parts.push(' (function(){ var refs=[]; if(primaryRef) refs.push(primaryRef); (extraRefs||[]).forEach(function(r){ if(r && (r.ts||r.section||r.version||r.release||r.ref)) refs.push(r); }); if(refs.length){ var lines = refs.map(function(r){ var url = "./preview/spec_viewer.html?ts="+encodeURIComponent(String(r.ts||"")) + "&version="+encodeURIComponent(String(r.version||"")) + "&section="+encodeURIComponent(String(r.section||"")) + "&ref="+encodeURIComponent(String(r.ref||"")) + "&release="+encodeURIComponent(String(r.release||"")); var label = String(r.ts||"") + (r.version?(" "+r.version):"") + (r.section?(" §"+String(r.section)):"" ) + (r.release?(" "+r.release):""); return "- ["+label+"]("+url+")"; }).join("\\n"); var refText = "References:\\n\\n" + lines; var injected=false; for(var i=0;i<combined.length;i++){ if(combined[i].indexOf("## Summary")===0){ combined[i] = combined[i] + "\\n\\n" + refText; injected=true; break; } } if(!injected){ combined.push(refText); } } })();');
  ...
  parts.push(' if(window.marked && window.marked.parse){ var raw = marked.parse(md || "(No additional documentation.)"); var safe = String(raw).replace(/<script[\\s\\S]*?<\\/script>/gi, "").replace(/ on[a-z]+=/gi, " "); commentContent.innerHTML = safe; } else { commentContent.textContent = md; }');
  parts.push(' });');
  ...
  parts.push(' if(labels.length){ select(0,false); }');
  parts.push(' });');
  parts.push(' })();');
  parts.push(closeScript);
  parts.push('</body>');
...
function DropZone(_ref15) {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    onMouseUp: function onMouseUp(e) {
      // 鍏滃簳锛氳嫢鑷姩鍖栨嫋鎷芥湭瑙︼拷??drop锛屽垯鐢ㄩ紶鏍囨姮璧锋墽??    
...
(function () {
  ...
  function buildHTML(nodes, exportMessages) {
    var messages = (exportMessages || []).map(function (m, i) {
      ...
      var header = {
        msg: h.msg || m.message || m.name || '',
        protocol: h.protocol || m.protocol || m.family || '',
        "interface": h["interface"] || m["interface"] || '',
        release: h.release || m.release || '',
        id: m.id || h.id || ''
      };
      if (!header["interface"]) header["interface"] = inferInterface(header.protocol);
      var id = header.id;
      var release = id && id.includes('@') ? id.split('@').pop() : header.release || 'Rel-18';
      header.release = release;
      var msgName = (header.msg || '').replace(/[_-]+/g, ' ').trim();
      ...
      return {
        ...
        info: {
          header: header,
          payload: (m.info && m.info.payload) || m.payload || {},
          ie_dict: (m.info && m.info.ie_dict) || m.ie_dict || {},
          ie_tree: (m.info && m.info.ie_tree) || m.ie_tree || [],
          overrides: (m.info && m.info.overrides) || m.overrides || {},
          primary_ref: (m.info && m.info.primary_ref) || m.primary_ref
        },
        comment: (m.info && m.info.doc && m.info.doc.comment_md) || (m.doc && m.doc.comment_md) || m.comment || ''
      };
    });
    var html = TEMPLATE.replace('__FLOW_JSON__', JSON.stringify({
      nodes: nodes,
      messages: messages
    }));
    if (html.indexOf('marked.umd.min.js') < 0) {
      html = html.replace('</head>', ' <script src="https://cdnjs.cloudflare.com/ajax/libs/marked/16.1.1/lib/marked.umd.min.js"></script>\n</head>');
    }
    if (html.indexOf('#comment-content code') < 0) {
      html = html.replace('</head>', ' <style>\n#comment-content{max-width:800px;margin:0 auto;word-wrap:break-word;overflow-wrap:break-word;}\n#comment-content code{font-family:Consolas,\"Courier New\",monospace;background-color:#f5f5f5;padding:2px 4px;border-radius:3px;}\n#comment-content pre code{background-color:transparent;padding:0;}\n#comment-content pre{background-color:#f5f5f5;padding:10px;border-radius:3px;overflow-x:auto;}\n#comment-content ul,#comment-content ol{margin:0.5em 0;padding-left:1.5em;}\n#comment-content li{margin:0.2em 0;}\n#comment-viewer::-webkit-scrollbar{width:8px;height:8px;}\n#comment-viewer::-webkit-scrollbar-thumb{background-color:rgba(0,0,0,0.3);border-radius:4px;}\n#comment-viewer::-webkit-scrollbar-track{background-color:#f5f5f5;}\n.message-label.selected{font-weight:bold;fill:#007ACC;}\n</style>\n</head>');
    }
    return html;
  }
})();
...
;(function(){
  try{
    if(!window.marked){
      ...
      s.src='https://cdnjs.cloudflare.com/ajax/libs/marked/16.1.1/lib/marked.umd.min.js';
      s.async=true;
