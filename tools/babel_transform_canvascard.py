...
def main():
    ...
    start = text.find('function CanvasCard')
    end = text.find('function RoleSelectRestricted')
    if start == -1 or end == -1 or end <= start:
        print('Failed to locate CanvasCard/RoleSelectRestricted anchors:', start, end)
        return
    ...
    try:
        ...
        print('CanvasCard subset transform OK')
    except py_mini_racer.JSEvalException as exc:
        print('CanvasCard subset transform failed:')
        print(exc)
